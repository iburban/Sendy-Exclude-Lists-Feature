...
<?php include('includes/functions_exclude_lists.php');?>
<?php 	
	//setup cron
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
	    {
			...
			if((($time>=$send_date && $time<$send_date+300) && $sent=='') || (($send_date<$time) && $sent=='') || ($send_date=='0' && $timezone=='0'))
			{
				...
				$q = 'SELECT id, name, email, list, custom_fields FROM subscribers WHERE list in ('.$email_list.') '.$exclude_lists_not_in_sql.' AND unsubscribed = 0 AND bounced = 0 AND complaint = 0 AND confirmed = 1 GROUP BY email  ORDER BY id ASC LIMIT 18446744073709551615'.$the_offset;
				$r = mysqli_query($mysqli, $q);
				if ($r && mysqli_num_rows($r) > 0)
				{
					$subscriber_id = '';
