...
if (isset($_POST['email_list_exclude']) && is_array($_POST['email_list_exclude']) && count($_POST['email_list_exclude']) > 0)
{
	$exclude_lists_arr = $_POST['email_list_exclude'];
	$exclude_lists_update_sql = ', exclude_lists = "'.implode(',', $exclude_lists_arr).'" ';
	$exclude_lists_not_in_sql = get_exclude_list_user_id_sql($email_list_array, $exclude_lists_arr);
}
...
if($cron)
{
	...
	$q = 'SELECT id FROM subscribers WHERE list in ('.$email_list.') '.$exclude_lists_not_in_sql.' AND unsubscribed = 0 AND bounced = 0 AND complaint = 0 AND confirmed = 1 GROUP BY email';
...
if($offset=='')
{
	...
	if ($r)
	{
		...
		$q2 = 'UPDATE campaigns SET to_send = '.$to_send.', to_send_lists = "'.$email_list.'" '.$exclude_lists_update_sql.' WHERE id = '.$campaign_id;
...
$q = 'SELECT id, name, email, list, custom_fields FROM subscribers WHERE list in ('.$email_list.') '.$exclude_lists_not_in_sql.' AND unsubscribed = 0 AND bounced = 0 AND complaint = 0 AND confirmed = 1 AND userID = '.get_app_info('main_userID').' GROUP BY email ORDER BY id ASC LIMIT 18446744073709551615'.$the_offset;
$r = mysqli_query($mysqli, $q);
if ($r && mysqli_num_rows($r) > 0)
{
	$subscriber_id = '';
